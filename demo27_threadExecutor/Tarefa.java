...
// Tarefa - Representa uma tarefa a ser executada, simulando um processamento com tempo aleatório
public class Tarefa implements Runnable {
    private Date dataInicio; // Data de criação da tarefa
    private String nome; // Nome da tarefa

    // Construtor: Inicializa a tarefa com um nome e registra a data de criação
    ...
    public void run() {
        ...
        try {
            // Simula o processamento com um tempo aleatório (em segundos)
            Long duracao = (long) (Math.random() * 10);
            System.out.printf("%s: Tarefa %s: Processando por %d segundos\n", Thread.currentThread().getName(), nome,
                    duracao);
